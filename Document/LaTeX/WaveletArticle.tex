...
% workaround for minted to work when output directory is not current.
% NOTE: Change output to your outputdir, or remove it.
\usepackage[outputdir=output]{minted} % Requires pygmentize and python-pygmentize.
\setminted[c++]{%
  % frame=single,
  fontsize=\small,
  style=borland,
...
%% If no access to minted use listings
% \usepackage{listings}
% \lstset{language=C++,
...
%         frame=tblr, framerule=0pt,
%         framextopmargin=3pt,
% }
%% columns fixed to avoid extra spaces
% \usepackage{realboxes}
% \newcommand{\inline}[1]{\Colorbox{light-gray}{\lstinline[columns=fixed]{#1}}}
...
\begin{abstract}
\noindent
This document describes the implementation of a multi-resolution analysis framework using isotropic and steerable wavelets in the frequency domain.
...
The backbone of the multi-scale analysis is provided by the Isotropic Wavelets, and the detection of directional features is provided by coupling it with a generalized Riesz transform.
...
\begin{enumerate}
  ...
  \item Multi-scale methods, pyramid, Meyer. Analysis and reconstruction.
\end{enumerate}
...
\begin{figure}[H]
  ...
  \begin{subfigure}[t]{.45\textwidth}
    \centering
    \includegraphics[width=1.0\textwidth]{profileMotherWavelet_Vow_5_Mother.png}
  ...
  \begin{subfigure}[t]{.45\textwidth}
    \centering
    \includegraphics[width=1.0\textwidth]{profileMotherWavelet_Held_5_Mother.png}
    \captionsetup{width=\textwidth}
    \caption{Held.}
  ...
  \begin{subfigure}[t]{.45\textwidth}
    \centering
    \includegraphics[width=1.0\textwidth]{profileMotherWavelet_Simoncelli_5_Mother.png}
    \captionsetup{width=\textwidth}
    \caption{Simoncelli.}
  ...
  \begin{subfigure}[t]{.45\textwidth}
    \centering
    \includegraphics[width=1.0\textwidth]{profileMotherWavelet_Shannon_5_Mother.png}
    \captionsetup{width=\textwidth}
    \caption{Shannon.}
...
\begin{figure}[H]
  ...
  \begin{subfigure}[t]{.45\textwidth}
    \centering
    \includegraphics[width=\textwidth]{profileMotherWavelet_Vow_5_SubBands.png}
    \captionsetup{width=\textwidth}
    \caption{Vow.}
  ...
  \begin{subfigure}[t]{.45\textwidth}
    \centering
    \includegraphics[width=\textwidth]{profileMotherWavelet_Held_5_SubBands.png}
    \captionsetup{width=\textwidth}
    \caption{Held.}
  ...
  \begin{subfigure}[t]{.45\textwidth}
    \centering
    \includegraphics[width=\textwidth]{profileMotherWavelet_Simoncelli_5_SubBands.png}
    \captionsetup{width=\textwidth}
    \caption{Simoncelli.}
  ...
  \begin{subfigure}[t]{.45\textwidth}
    \centering
    \includegraphics[width=\textwidth]{profileMotherWavelet_Shannon_5_SubBands.png}
    \captionsetup{width=\textwidth}
    \caption{Shannon.}
...
\subsection{Multi-resolution: Image Pyramids}
...
There is no direct generalization of the analytical signal in ND, but we can generate a signal with similar properties, called the \textbf{Monogenic signal} using the Riesz transform of order 1\cite{felsberg_monogenic_2001, kovesi_peters_nodate}.
...
This kind of iterators add the functions GetFrequencyBin(), GetFrequencyIndex() to a regular \doxygen{ImageRegionIterator}, helping to abstract the complexity of the frequency layout into the iterator.
...
Although, when applying a \doxygen{ForwardFFTImageFilter}, the output image, which is now in the frequency domain, still have metadata referring to the spatial domain, but nothing about the frequency origin, or spacing between frequencies.
...
\label{sub:wavelet_transform}

The next section deals with the Wavelet Pyramid that generates wavelet coefficients for each level (equal to the number of M-bands per level, more later).  The pyramid is redundant, and perfect reconstruction of the original input can be achieved using the inverse Wavelet transform when the wavelet function is isotropic

The Wavelet Transform is templated over an IsotropicWavelet chosen by the user through a \github{WaveletFrequencyFilterBankGenerator}
...
\label{sub:riesz_transform}


\subsection{Structure Tensor}
\label{sub:structure_tensor}

Given an array of inputs, StructureTensor computes the linear combination (or direction) of inputs that maximizes the response for each location in the image. Instead of only measuring the response at the pixel of interest, it takes into account a local neighborhood. \cite{unser_steerable_2011}.
...
You can apply a \doxygen{FFTPadImageFilter} to pad the image with zeros to reach the required size. However, this filter sets some padded areas with negative indices, which can generate problems with neighbour iterators.
...
\label{sub:Choosing}
Choose a mother wavelet from the options available \ref{sub:Isotropic}, and create a
...
\begin{figure}[H]
  \begin{subfigure}[t]{.49\textwidth}
    \centering
    \includegraphics[width=\textwidth]{phaseImages/checker_original512x512.png}
    \captionsetup{width=\textwidth}
    \caption{Original}
  ...
  \begin{subfigure}[t]{.49\textwidth}
    \centering
    \includegraphics[width=\textwidth]{phaseImages/checker_grayscale.png}
    \captionsetup{width=\textwidth}
...
\begin{figure}[H]
  \begin{subfigure}[t]{.333\textwidth}
    \centering
    \includegraphics[width=\textwidth]{phaseImages/held2D_1_1.png}
    \captionsetup{width=\textwidth}
    \caption{Levels: 1}
    \label{fig:phase11}
  \end{subfigure}
  \begin{subfigure}[t]{.333\textwidth}
    \centering
    \includegraphics[width=\textwidth]{phaseImages/held2D_2_1.png}
    \captionsetup{width=\textwidth}
    \caption{Levels: 2}
    \label{fig:phase21}
  \end{subfigure}
  \begin{subfigure}[t]{.333\textwidth}
    \centering
    \includegraphics[width=\textwidth]{phaseImages/held2D_3_1.png}
    \captionsetup{width=\textwidth}
    \caption{Levels: 3}
    \label{fig:phase31}
  \end{subfigure}
  ...
  \begin{subfigure}[t]{.333\textwidth}
    \centering
    \includegraphics[width=\textwidth]{phaseImages/held2D_4_1.png}
    \captionsetup{width=\textwidth}
    \caption{Levels: 4}
    \label{fig:phase41}
  \end{subfigure}
  \begin{subfigure}[t]{.333\textwidth}
    \centering
    \includegraphics[width=\textwidth]{phaseImages/held2D_5_1.png}
    \captionsetup{width=\textwidth}
    \caption{Levels: 5}
    \label{fig:phase51}
  \end{subfigure}
  \begin{subfigure}[t]{.333\textwidth}
    \centering
    \includegraphics[width=\textwidth]{phaseImages/held2D_6_1.png}
    \captionsetup{width=\textwidth}
    \caption{Levels: 6 }
    \label{fig:phase61}
  \end{subfigure}
...
\begin{figure}[H]
  ...
  \begin{subfigure}[t]{.333\textwidth}
    \centering
    \includegraphics[width=\textwidth]{phaseImages/held2D_6_2.png}
    \captionsetup{width=\textwidth}
  ...
  \begin{subfigure}[t]{.333\textwidth}
    \centering
    \includegraphics[width=\textwidth]{phaseImages/held2D_6_3.png}
    \captionsetup{width=\textwidth}
  ...
  \begin{subfigure}[t]{.333\textwidth}
    \centering
    \includegraphics[width=\textwidth]{phaseImages/held2D_6_4.png}
    \captionsetup{width=\textwidth}
  ...
  \begin{subfigure}[t]{.333\textwidth}
    \centering
    \includegraphics[width=\textwidth]{phaseImages/held2D_6_5.png}
    \captionsetup{width=\textwidth}
  ...
  \begin{subfigure}[t]{.333\textwidth}
    \centering
    \includegraphics[width=\textwidth]{phaseImages/held2D_6_10.png}
    \captionsetup{width=\textwidth}
    \caption{HighPassSubBands: 10}
    \label{fig:phase_bands610}
  \end{subfigure}
  \caption{Using the maximum number of scales (Level: 6), results for different number of high frequency sub-bands.}
...
\begin{figure}[H]
  ...
  \begin{subfigure}[t]{.4\textwidth}
    \centering
    \includegraphics[width=0.8\textwidth]{phaseImages/hermann_simoncelli_8_10.png}
    \captionsetup{width=\textwidth}
...
This work provides ITK with a multi-resolution analysis based on wavelet decomposition using an isotropic and steerable wavelets. Also it provides utilities to work in the dual or frequency domain. Frequency iterators, shrinkers and expanders. It also provides a sub-sampler without interpolation, and a expander with zeros that work in any domain.

As an application, we implement a local phase analyzer using the wavelet coefficients based on \cite{held_steerable_2010}.

Future work would be easier to implement with the tools already developed; this include the Simoncelli Steerable Framework \cite{simoncelli_steerable_1995}, and the more general Steerable framework using Riesz transform \cite{unser_steerable_2011}. Denoising algorithms can be developed using the exposed wavelet coefficients, for example: Gaussian Mixture Models (GMM)\cite{portilla_image_2003}, or SURE-LET \cite{blu_sure-let_2007}. Wavelets can also be used to feature detection without using template matching in an efficient way \cite{puspoki_template-free_2015}. Implementation of these algorithms will provide state of the art solutions based on wavelets to a wider audience, and I am happy to link to them from the External Repository. \url{https://github.com/phcerdan/ITKIsotropicWavelets}.
